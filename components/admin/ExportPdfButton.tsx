...
export default function ExportPdfButton({ employeeName, cycleTitle, results }: Props) {
  ...
  async function generate() {
    ...
    for (const rel of ['SELF', 'MANAGER', 'PEER', 'DIRECT_REPORT']) {
      ...
      checkY(14)
      doc.setFontSize(12).setTextColor(30).setFont(undefined, 'bold')
      doc.text(REL_LABELS[rel] ?? rel, margin, y)
      y += 6

      if (!section.visible) {
        doc.setFontSize(9).setTextColor(130).setFont(undefined, 'normal')
      ...
      for (const q of section.questions) {
        if (q.category !== lastCat) {
          checkY(10)
          doc.setFontSize(8).setTextColor(150).setFont(undefined, 'bold')
        ...
        doc.setFontSize(9).setTextColor(40).setFont(undefined, 'normal')
        doc.text(lines, margin + 2, y); y += lines.length * 4.5

        if (q.type === 'RATING' && q.average !== undefined) {
          doc.setFontSize(11).setTextColor(245, 78, 0).setFont(undefined, 'bold')
