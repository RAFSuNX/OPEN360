  ...
  return (
    <div>
      ...
      <div className="skeleton-card" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '10px 16px', background: 'var(--canvas-soft)', borderBottom: '1px solid var(--hairline)', display: 'flex', gap: '40px' }}>
          {[100, 150, 80, 100, 80].map((w, i) => (
            <div key={i} className="skeleton skeleton-text" style={{ width: `${w}px` }} />
          ))}
        </div>
        {[0,1,2,3,4,5].map(i => (
          <div key={i} style={{ padding: '14px 16px', borderBottom: '1px solid var(--hairline-soft)', display: 'flex', gap: '40px', alignItems: 'center' }}>
